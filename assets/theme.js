...
var icecream_variants_list = [32151691952193, 32118897836097, 32118897901633, 32118897803329, 32118897868865, 32118897934401, 32170021093441, 32170021126209, 32170021158977, 32170021257281, 32170021191745, 32170021224513, 32170021290049, 32198293815361, 32198293848129, 32198293880897, 4617061007425, 4614620250177, 4614620840001]
var icecream_variants_bundles_list = [32151691952193, 32118897836097, 32118897901633, 32167468105793, 32167468499009, 32170021158977, 32170021257281, 32170021224513, 32170021290049]
...
function check_case_ReCharge(variant) {
    if (variant.title == "Pack of 6" || variant.title == "Pack of 12" || variant.title  == "4 Pack / No Zoku Bowl" || variant.title == "6 Pack / No Zoku Bowl") {
      if($('#rc_container').length > 0) {
          $('#rc_elmhurst1925').show();
        };
    }
    else {
      if($('#rc_container').length > 0) {
        var targetDiv = document.getElementsByClassName("rc_block__type__onetime")[0].getElementsByClassName("rc_label__onetime")[0];
        targetDiv.click();
        $('#rc_elmhurst1925').hide();
      };
    }
  };
...
theme.Slideshow = (function() {
  ...
  function slideshow(el, sectionId) {
    ...
    this.$play = this.$controls.find(selectors.playButton);
...
theme.Product = (function() {
  function Product(container) {
    ...
    this.selectors = {
      ...
      salePricePromoPre: '[data-sale-price-promo-pre]',
      salePricePromoPost: '[data-sale-price-promo-post]',
      regularPricePromoPre: '[data-regular-price-promo-pre]',
      regularPricePromoPost: '[data-regular-price-promo-post]'
    };
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _updateAddToCart: function(evt) {
      ...
      if (variant) {
        if (variant.available) {
          ...
          var is_icecream = icecream_variants_list.includes(variant.id);

          // if (is_icecream) {$(this.selectors.addToCartText).text('Available Soon');}
    ...
    _updateImages: function(evt) {
      ...
      var is_icecream_bundle = icecream_variants_bundles_list.includes(variant.id);

      if (is_icecream_bundle && !variant.available) {
        $('.product-single__photos').css({"filter":"grayscale(100%)"});
      }
      else {
        $('.product-single__photos').css({"filter":"none"});
      }
    ...
    _updatePrice: function(evt) {
      ...
      var $regularPricePromoPre = $(this.selectors.regularPricePromoPre, $priceContainer);
      var $regularPricePromoPost = $(this.selectors.regularPricePromoPost, $priceContainer);
      var $salePrice = $(this.selectors.salePrice, $priceContainer);
      var $salePricePromoPre = $(this.selectors.salePricePromoPre, $priceContainer);
      var $salePricePromoPost = $(this.selectors.salePricePromoPost, $priceContainer);
      ...
      if (variant.compare_at_price > variant.price) {
        ...
        $regularPricePromoPre.html('Retail value if purchased separately: ');

        var salePricePromoCalc = Math.round((variant.compare_at_price - variant.price) * 100 /  variant.compare_at_price)
        salePricePromoCalc = Math.round(salePricePromoCalc / 5) * 5
        $salePricePromoPre.html('Bundle Price: ');
        $('.price__sale').css("color","#e61417");
        $salePricePromoPost.html(' (You Save ' + salePricePromoCalc + '%)');
        $('.ice_cream_supplies_last').css("display", "block");
        ...
        $priceContainer.addClass(this.classes.productOnSale);
        var salePricePromoCalc = Math.round((variant.compare_at_price - variant.price) * 100 /  variant.compare_at_price)
        $salePricePromoPost.html('(' + salePricePromoCalc + '% Savings)');
        ...
        $regularPricePromoPost.html('');
        $salePricePromoPre.html('');
        $regularPricePromoPre.html('');
...
$(document).ready(function() {
  ...
  return url.replace(/^.*\/([^?#\/]+).*$/, function (_, word) {
   ...
   var change =  word.charAt(0).toUpperCase() + word.slice(1);
     res = change.split("-");
    var getProduct = res[res.length - 1]
   var checkVal = getProduct.slice(0, -1)
   console.log(checkVal)
   $(`.custom_variants_images .child`).removeClass('active')
   $(`.custom_variants_images .child[data-check="${checkVal}"]`).addClass('active')
  });
}
  
  lastWordCapitalized(window.location.href)
  if(window.location.href.indexOf("soft-serve-chocolate") > -1) {
    setTimeout(function(){$('.PwzrGrid-align-items-xs-center').hide();},100)
	}
  if(window.location.href.indexOf("soft-serve-vanilla") > -1) {
    setTimeout(function(){$('.PwzrGrid-align-items-xs-center').hide();},100)
	}
  if(window.location.href.indexOf("soft-serve-variety-pack") > -1) {
    setTimeout(function(){$('.PwzrGrid-align-items-xs-center').hide();},100)
	}
});
